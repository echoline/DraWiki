...
<head>
	...
	print '/';
	$url = substr(preg_replace("/ /", "%20", htmlentities(preg_replace("/\?.*$/", "", $_SERVER['REQUEST_URI']))),1);
	print $url;
...
?>
<div style="float:left;"><p><input type="text" style="width:33%" placeholder="<?php

	if (htmlentities($_SERVER['REQUEST_URI']) == '/')
		echo "Name your canvas here";
	else
		echo preg_replace("/%20/", " ", $url);
